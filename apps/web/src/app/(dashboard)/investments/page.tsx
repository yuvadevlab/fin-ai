...
export default function InvestmentsPage() {
  const total = React.useMemo(() => investments.reduce((s, a) => s + a.value, 0), []);
  ...
  const columns = React.useMemo(
    () => [
      {
        header: "Asset",
        accessor: (a: any) => a.name,
        className: "font-semibold",
      },
      {
        header: "Value",
        accessor: (a: any) => <MoneyDisplay value={a.value} />,
    ...
    <PageContainer>
      ...
      <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <DataTable data={investments} columns={columns} rowKey={(inv: any) => inv.name} />
        </div>
      </section>
    </PageContainer>
  );
